...
class Server(Actor):
    ...
    def train(self, selected_nodes):
        ...
        if self.downlink[0].actor_type == 'client':  # i.e. FedAvg
            ...
            trainable, valid_nodes = self.check_selected_trainable(
                selected_nodes)
            if trainable == True:
            ...
            # Check the trainable of all groups，这一步要根据概率选择group
            trainable, valid_nodes = self.check_selected_trainable(
                self.downlink)
            if trainable == True:
                for group in valid_nodes:
                    # selected clients不能提前确定，也要在group里面选，这里需要重新写一下逻辑
                    group_num_samples, group_train_acc, group_train_loss, soln, group_update = group.train(
                        selected_nodes)
                    results.append(
                        [group, group_num_samples, group_train_acc, group_train_loss, group_update])

        if results == []:
            print(
                'ERROR: This server has not training clients/groups with training data/clients')
            return
        return results
    ...
    def test(self, selected_nodes):
        ...
        if testable == True:
            ...
            print(
                'ERROR: This server has not test clients/groups with testing data/clients')
            return
