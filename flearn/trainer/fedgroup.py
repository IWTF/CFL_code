...
class FedGroup(GroupBase):
    ...
    def clustering_clients(self, clients, n_clusters=None, max_iter=20):
        ...
        for c in clients:
            _, _, _, csolns[c], cupdates[c] = c.pretrain(
                self.init_params, iterations=50)
        ...
        print("EDC Matrix calculation takes {}s seconds".format(
        ...
        MADC = self._calculate_data_driven_measure(
            full_cossim_matrix, correction=True)  # shape=(n_clients, n_clients)
        print("MADC Matrix calculation takes {}s seconds".format(
            time.time()-start_time))
        ...
        if self.measure == 'MADC':
            ...
            result = AgglomerativeClustering(
                n_clusters, affinity='precomputed', linkage='complete').fit(affinity_matrix)
        if self.measure == 'EDC':  # Use EDC as default
            ...
            result = KMeans(n_clusters, random_state=self.seed,
    ...
    def _calculate_data_driven_measure(self, pm, correction=False):
        ...
        absdiff_pm_matrix = np.abs(
            col_pm_matrix - row_pm_matrix)  # shape=(n_clients, n_clients, n_clients)
        ...
        if correction == True:
            ...
            for idx in range(mask.shape[-1]):
                ...
                # Mask all 0->n colum for 0->n diff matrix,
                mask[idx, :, idx] = 1
    ...
    def schedule_clients(self, round, selected_clients, groups):
        ...
        if self.dynamic == True:
            ...
            warm_clients = [
                wc for wc in self.clients if wc.has_uplink() == True]
    ...
    def schedule_groups(self, round, clients, groups):
        if self.dynamic == True and self.recluster_epoch is not None:
            ...
            if round in self.recluster_epoch:
                file = open('./fedgroup.txt', 'a+')
                file.write(f'{round} 世界。\n')
                file.close()
    ...
    def reassign_clients_by_temperature(self, clients, metrics, func):
        ...
        def _lied_temperature(client_bias, group_bias, temp, max_temp):
            ...
            if client_bias > group_bias:  # Temperature exponential decrease
                ...
                scale = min(client_bias / (group_bias + 1e-5),
                            10)  # Prevent Overflow
        ...
        def _eied_temperature(client_bias, group_bias, temp, max_temp):
            ...
            abs_bias = min(abs(client_bias - group_bias),
                           10)  # Prevent Overflow
            rate = 2 * max_temp / (max_temp - 1)
        ...
        for wc in [c for c in clients if c.has_uplink() == True and c.temperature is not None]:
            ...
            if func == 'step':
                wc.temperature = _step_temperature(
                    client_bias, group_bias, wc.temperature, wc.max_temp)
            if func == 'linear':
                wc.temperature = _linear_temperature(
                    client_bias, group_bias, wc.temperature, wc.max_temp)
            if func == 'lied':
                wc.temperature = _lied_temperature(
                    client_bias, group_bias, wc.temperature, wc.max_temp)
            if func == 'eied':
                wc.temperature = _eied_temperature(
                    client_bias, group_bias, wc.temperature, wc.max_temp)
    ...
    def schedule_clients_after_training(self, comm_round, clients, groups):
        ...
        self.reassign_clients_by_temperature(
            clients, self.temp_metrics, self.temp_func)
        return
