...
class GroupBase(object):
    def __init__(self, train_config):
        ...
        self.group_config.update({'eval_locally': self.eval_locally})
    ...
    def construct_actors(self):
        ...
        if self.eval_global_model == True:
            ...
            # Make the test data in server have the same size of local dataset, and data distribution is random
            # self.server.test_data['x'] = random.sample(
            #     self.server.test_data['x'], len(test_data[clients[0]]['x']))
            # self.server.test_data['y'] = random.sample(
            #     self.server.test_data['y'], len(test_data[clients[0]]['y']))
    ...
    def inter_group_aggregation(self, train_results, agg_lr=0.0):
        ...
        for idx, g in enumerate(groups):
            ...
            inter_aggregation_update = [w1-w0 for w0,
                                        w1 in zip(g.latest_params, averaged_soln)]
            g.latest_updates = [
                up0+up1 for up0, up1 in zip(g.latest_updates, inter_aggregation_update)]
    ...
    def select_clients(self, comm_round, num_clients=20):
        ...
        # Round robin，为了让所有client至少参与一次，不能要
        if comm_round < len(self.clients) / num_clients:
            ...
            if head + num_clients <= len(self.clients):
                ...
                selected_clients = self.clients[head:] + \
                    self.clients[:head+num_clients-len(self.clients)]
        # Random selecte clients
        else:
            # make sure for each comparison, we are selecting the same clients each round
            random.seed(comm_round+self.seed)
    ...
    def weighted_aggregate(self, updates, weights):
        ...
        normalws = np.array(weights, dtype=float) / \
            np.sum(weights, dtype=np.float)
        num_clients = len(updates)
        num_layers = len(updates[0])
        agg_updates = []
        for la in range(num_layers):
            agg_updates.append(
                np.sum([up[la]*pro for up, pro in zip(updates, normalws)], axis=0))
    ...
    def summary_results(self, comm_round, train_results=None, test_results=None):
        ...
        if partial_test_acc == True:
            summary = {'Total': (None, None, None)}
        ...
        if results[0][0].actor_type == 'group':
            ...
            for idx, g in enumerate(groups):
                if partial_test_acc == True:
                    accs[idx]
                # accuracy, loss, number of samples
                summary[f'G{g.id}'] = (accs[idx], losses[idx], nks[idx])
    ...
    def train_locally(self, num_epoch=20, batch_size=10):
        ...
        train_size, train_acc, train_loss, soln, update = self.server.solve_inner(
    ...
    def refresh_discrepancy_and_dissmilarity(self, clients):
        ...
        def _calculate_pnum_var(clients):
            p_nums = [c.participate_num for c in clients]
            # print("+++++++++++++++++participant num", p_nums)
            return np.var(p_nums)
        ...
        diffs = {}
        # diffs['Total'] = _calculate_mean_diffs(
        #     clients)[0]  # Return discrepancy
        diffs['Total'] = _calculate_pnum_var(self.clients)
        ...
        for g in groups:
            ...
            g.pnum_var = _calculate_pnum_var(gc)
            # 这里我不需要discrepancy，所以把数值换成pnum_var
            # i.e. { 'G1': (numer of group clients, discrepancy) }
            diffs[f'G{g.id}'] = (len(gc), g.pnum_var)
        return diffs
    ...
    def update_auxiliary_global_model(self, groups):
        ...
        new_server_params = self.simply_averaging_aggregate(
            [g.latest_params for g in groups])
    ...
    def swap_data(self, clients, swap_p, scope='all'):
        ...
        if swap_p > 0 and swap_p < 1:
            ...
            swap_flag = np.random.choice([0, 1], int(
                clients_size/2), p=[1-swap_p, swap_p])  # Half size of clients_size
            for idx in np.nonzero(swap_flag)[0]:
                ...
                if scope == 'all':
                    ...
                    if g2 != g1:
                        print(colored(
                            f"Swap C-{c1.id}@G{g1.id} and C-{c2.id}@G{g2.id} data", 'cyan', attrs=['reverse']))

                if scope == 'part':
                    if len(c1.label_array) == 0 or len(c2.label_array) == 0:
                        return
                    ...
                    if c1_diff.size == 0 or c2_diff.size == 0:
                        return
                    # np.random.choice 从数组c1_diff中随机选择一个
                    c1_swap_label, c2_swap_label = np.random.choice(
                        c1_diff, 1)[0], np.random.choice(c2_diff, 1)[0]
                    c1.distribution_shift, c2.distribution_shift = True, True
                    '''
                    print('Debug', np.unique(
                        c1.train_data['y']), np.unique(c1.test_data['y']))
                    print('Debug', np.unique(
                        c2.train_data['y']), np.unique(c2.test_data['y']))
                    ...
                    for c1_data, c2_data in zip([c1.train_data, c1.test_data], [c2.train_data, c2.test_data]):
                        ...
                        # Swap the feature
                        c1_data['x'] = np.delete(
                            c1_data['x'], label_idx1, axis=0)
                        c1_data['x'] = np.vstack([c1_data['x'], c2_swap_x])
                        c2_data['x'] = np.delete(
                            c2_data['x'], label_idx2, axis=0)
                        ...
                        # Shuffle the data
                        random_idx1, random_idx2 = np.arange(
                            c1_data['y'].shape[0]), np.arange(c2_data['y'].shape[0])
                        np.random.shuffle(
                            random_idx1), np.random.shuffle(random_idx2)
    ...
    def increase_data(self, round, clients):
        ...
        if round in processing_round:
            ...
            print('>Round {:3d}, {:.1%} training data release.'.format(
                round, release_rate))
            for c in clients:
