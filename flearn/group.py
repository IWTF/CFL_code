...
class Group(Actor):
    def __init__(self, id, config, train_data={'x': [], 'y': []}, test_data={'x': [], 'y': []}, uplink=[], model=None):
        actor_type = 'group'
        super(Group, self).__init__(id, actor_type,
                                    train_data, test_data, model=model)
        if len(uplink) > 0:
        ...
        # The mean cosine dissimilarity between this group and sublink node
        self.cosine_dissimilarity = 0

        self.discrepancy_g = 0  # The mean discrepancy between this group and gobal model
        # The mean cosine dissimilarity between this group and gobal model
        self.cosine_dissimilarity_g = 0
    ...
    def update_difference(self):
        ...
        self.discrepancy_g = _calculate_l2_distance(
            self.latest_params, self.uplink[0].latest_params)
        self.cosine_dissimilarity_g = calculate_cosine_dissimilarity(
            self.latest_params, self.uplink[0].latest_params)
        return self.cosine_dissimilarity_g
        ...
        if len(temps) == 0:
            ...
            for nk, temp in zip(nks, temps):
                if temp == None:
                    ...
                    temp_nks.append(
                        floor((max(temp, 0) / (max_temp+epsilon)) * nk))
    ...
    def weighted_aggregate(self, updates, weights):
        ...
        normalws = np.array(weights, dtype=float) / \
            (np.sum(weights, dtype=np.float) + epsilon)
    ...
    def train(self, selected_nodes=None):
        ...
        if trainable == True:
            ...
            for node in valid_nodes:
                ...
                node.latest_updates = [
                    (w1-w0) for w0, w1 in zip(node.latest_params, group_params)]
                node.latest_params = group_params

                # init the alpha params FedADMM
                temp = []
                for i in range(len(self.latest_params)):
                    temp.append([])
                    temp[i] = tf.zeros_like(self.latest_params[i])
                self.alpha.append(temp)

            # 2, Train the neural model of client and save the results
            for idx, node in enumerate(valid_nodes):
                # FedCCS
                if self.aggregation_strategy == 'fedcos':
                    num_samples, train_acc, train_loss, soln, update = node.train(
                        self.alpha[idx], self.latest_params)
                else:
                    num_samples, train_acc, train_loss, soln, update = node.train()
                solns.append(soln)
                train_results.append(
                    [node, num_samples, train_acc, train_loss, update])
            ...
            if self.aggregation_strategy == 'temp' and max_temp is not None:
                agg_updates = self.federated_averaging_aggregate_with_temperature(
                    updates, nks, temps, max_temp)
            if self.aggregation_strategy == 'fedavg':
                agg_updates = self.federated_averaging_aggregate(updates, nks)
            if self.aggregation_strategy == 'avg':
                agg_updates = self.federated_averaging_aggregate(
                    updates, [1.0*len(nks)])
            if self.aggregation_strategy == 'fedcos':
                agg_updates = self.federated_averaging_aggregate(updates, nks)
            ...
            group_train_acc = np.average(
                [rest[2] for rest in train_results], weights=nks)
            group_train_loss = np.average(
                [rest[3] for rest in train_results], weights=nks)
            ...
            print(
                f'ERROR: Group {self.id} has not any valid training clients with training data which is invalid.')
            return
    ...
    def test(self):
        ...
        if self.eval_locally == False:
            ...
            group_num_samples = np.sum(nks, dtype=np.float)
            group_test_acc = np.average(
                [rest[1] for rest in test_results], weights=nks)
            group_test_loss = np.average(
                [rest[2] for rest in test_results], weights=nks)
        else:
