...
def read_cifar(train_data_dir, test_data_dir):
...
def read_federated_data(dsname):
    ...
    # The working path of SplitGP
    wspath = Path(__file__).parent.parent.absolute()
    # The training data directory
    train_data_dir = Path.joinpath(
        wspath, 'data', dsname, 'data', 'train').absolute()
    # The testing data directory
    test_data_dir = Path.joinpath(
        wspath, 'data', dsname, 'data', 'test').absolute()

    if dsname == 'cifar':
        clients, train_data, test_data = read_cifar(
            train_data_dir, test_data_dir)
    if dsname.startswith('mnist'):
        clients, train_data, test_data = read_mnist(
            train_data_dir, test_data_dir)
    if dsname == 'femnist':
        clients, train_data, test_data = read_femnist(
            train_data_dir, test_data_dir)
    if dsname == 'fmnist':
        clients, train_data, test_data = read_fmnist(
            train_data_dir, test_data_dir)
    if dsname.startswith('synthetic'):
        clients, train_data, test_data = read_synthetic(
            train_data_dir, test_data_dir)
    if dsname == 'sent140':
        max_words = 25
        emb_file = Path.joinpath(
            wspath, 'data', dsname, 'embs.json').absolute()
        clients, train_data, test_data = read_sent140(
            train_data_dir, test_data_dir)
    ...
    for c in train_data.keys():
        ...
        train_data[c]['y'] = np.array(
            train_data[c]['y'], dtype=np.uint8)  # shape=(num_samples, )
    ...
    print(
        f'The dataset size: {train_size + test_size}, train size: {train_size}, test size: {test_size}.')
    print(
        f'The train client count: {len(train_data)}. The test client count: {len(test_data)}.')
