...
class ResultWriter(object):
    ...
    def make_header(self):
        if self.trainer_type == 'fedavg':
            header = ['TestAcc', 'TrainAcc',
                      'TrainLoss', 'NumClient', 'Discrepancy']
        if self.trainer_type in ['fedcos', 'fedgroup', 'ifca', 'fesem']:
            header = ['WeightedTestAcc', 'WeightedTrainAcc',
                      'WeightedTrainLoss', 'NumGroup', 'Discrepancy']
            if self.migration == True:
                header += ['Shift', 'Migration']
            for gid in range(self.num_group):
                header += [f'G{gid}TestAcc', f'G{gid}TrainAcc',
                           f'G{gid}TrainLoss', f'G{gid}Diff', f'G{gid}NumClinet']
        return header
    ...
    def make_filename(self, config):
        ...
        if shift_type in ['all', 'part']:
            ...
            if swap > 0 and swap < 1:
                filename += f'-{shift_type}_swap{swap}'
        ...
        if trainer_type == 'fedgroup':
            ...
            filename = filename + \
                f'-FG{num_group}-{measure}-agglr{agglr}-tempagg{temp_agg}'
            if dynamic == True:
                if temp is not None:
                    filename += f'-TEMP{temp}-{temp_metrics}-{temp_func}'
            else:
                filename += '-static'
            if RAC == True:
                filename += '-RAC'
